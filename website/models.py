...
class Plot_data_point(models.Model):
    ...
    x_data = models.DecimalField(decimal_places=2)
    y_data = models.DecimalField(decimal_places=6)

class Results_from_mkclass(models.Model):
	filename = models.CharField(max_length=500)
	resulting_class = models.CharField(max_length=100)
	resulting_quality = model.CharField(max_length=30)
	calc_date = models.DateTimeField()

class Template_choice(model.Model):
